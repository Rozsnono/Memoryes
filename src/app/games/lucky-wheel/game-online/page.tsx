...
function OnlineGameContent() {
    ...
    useEffect(() => {
        ...
        const pollGame = async () => {
            try {
                ...
                // Only update state if we are NOT the one currently acting (to prevent UI jitter)
                setLobby(data.data);
                setLoading(false);
            } catch (err) {
    ...
    const handleSpinComplete = (segment: WheelSegment) => {
        ...
        if (segment.value === 'LOST') {
            ...
            pushStateUpdate({ playerScores: lobby.playerScores.map((s, i) => i === lobby.activePlayerIdx ? 0 : s), activePlayerIdx: nextIdx, turnState: 'idle' });
    ...
    // --- RE-ENGINEERED SOLVE LOGIC (STAY IN MODE UNTIL WRONG) ---
    const handleSolveLetterSelect = async (letter: string) => {
        ...
        if (occurrences > 0) {
            ...
            if (isSolved) {
                const finalTotals = [...lobby.playerTotalScores];
                finalTotals[lobby.activePlayerIdx] += lobby.playerScores[lobby.activePlayerIdx];
                const isGameOver = (lobby.round || 1) >= (lobby.maxRounds || 3);

                await pushStateUpdate({
                    revealedLetters: nextRevealed,
                    playerTotalScores: finalTotals,
                    turnState: isGameOver ? 'game_over' : 'round_over'
                });
            } else {
                // STAY IN SOLVING MODE
                await pushStateUpdate({
                    revealedLetters: nextRevealed,
                    turnState: 'solving'
                });
                toast.success(`"${letter}" is correct! Keep going.`);
            }
        } else {
            // WRONG: Turn ends, go to IDLE, move to next player
            const nextIdx = (lobby.activePlayerIdx + 1) % lobby.players.length;
            await pushStateUpdate({
                revealedLetters: nextRevealed,
            ...
            toast.error(`Incorrect! Turn passed.`);
        }
    };
    ...
    const handleStartNextRound = async () => {
        ...
        try {
            ...
            await pushStateUpdate({
                phrase: selectedPuzzle.phrase, category: selectedPuzzle.category,
                revealedLetters: [], playerScores: lobby.players.map(() => 0),
                currentWheelValue: '', turnState: 'idle', round: (lobby.round || 1) + 1
            });
