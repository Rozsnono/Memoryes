...
function OnlineGameContent() {
    ...
    const pushStateUpdate = async (updates: Partial<SyncLobbyType>) => {
        if (!codeParam || !lobby) return;

        isUpdating.current = true;
        const newState = { ...lobby, ...updates };
        setLobby(newState);

        try {
            await apiClient.put(`/api/lucky-wheel/lobby/${codeParam}/`, updates);
        } catch (err) {
    ...
    // --- NEW: ROBUST BANKING LOGIC ---
    const finalizeRound = async (winningIdx: number, currentRevealed: string[], currentRoundScores: number[]) => {
        if (!lobby) return;

        // Calculate Bank: Add current round's points to the total
        const newTotalScores = [...lobby.playerTotalScores];
        const pointsToBank = currentRoundScores[winningIdx];
        newTotalScores[winningIdx] += pointsToBank;

        const isGameOver = (lobby.round || 1) >= (lobby.maxRounds || 3);

        await pushStateUpdate({
            revealedLetters: currentRevealed,
            playerTotalScores: newTotalScores, // SAVE THE BANK
            playerScores: lobby.players.map(() => 0), // CLEAR THE ROUND
            turnState: isGameOver ? 'game_over' : 'round_over'
        });

        toast.success(`Round Won! ${pointsToBank}$ added to your bank.`);
    };
    ...
    const handleSpinComplete = (segment: WheelSegment) => {
        ...
        if (segment.value === 'LOST') {
            ...
            pushStateUpdate({
                playerScores: lobby.playerScores.map((s, i) => i === lobby.activePlayerIdx ? 0 : s),
                activePlayerIdx: nextIdx,
                turnState: 'idle'
            });
            toast.error('Bankrupt!');
    ...
    const handleLetterSelect = async (letter: string, isVowel: boolean) => {
        if (!lobby) return;
        let nextScores = [...lobby.playerScores];

        if (isVowel) {
            const cost = 250;
            if (nextScores[lobby.activePlayerIdx] < cost) return toast.error("Not enough points");
            nextScores[lobby.activePlayerIdx] -= cost;
        }
        ...
        if (occurrences > 0) {
            ...
            const isSolved = lobby.phrase.toUpperCase().split('').every(char => char === ' ' || char === '-' || nextRevealed.includes(char));

            if (isSolved) {
                await finalizeRound(lobby.activePlayerIdx, nextRevealed, nextScores);
            } else {
            ...
            toast.error(`No "${letter}".`);
        }
    };

    const handleSolveLetterSelect = async (letter: string) => {
        if (!lobby) return;
        ...
        if (occurrences > 0) {
            ...
            if (isSolved) {
                await finalizeRound(lobby.activePlayerIdx, nextRevealed, lobby.playerScores);
            } else {
                pushStateUpdate({ revealedLetters: nextRevealed, turnState: 'solving' });
                toast.success(`"${letter}" is correct!`);
            }
        } else {
            const nextIdx = (lobby.activePlayerIdx + 1) % lobby.players.length;
            pushStateUpdate({ revealedLetters: nextRevealed, activePlayerIdx: nextIdx, turnState: 'idle' });
            toast.error(`Incorrect letter.`);
        }
    };
        ...
        try {
            ...
            activeThemes.forEach((theme: any) => {
                theme.puzzles.forEach((phrase: string) => {
                    if (phrase.toUpperCase() !== lobby.phrase.toUpperCase()) puzzlePool.push({ phrase, category: theme.title });
                });
            });
            const selectedPuzzle = puzzlePool[Math.floor(Math.random() * puzzlePool.length)] || puzzlePool[0];
            ...
            await pushStateUpdate({
                ...
                playerTotalScores: lobby.playerTotalScores, // ENSURE TOTALS ARE CARRIED OVER
                currentWheelValue: '',
                turnState: 'idle',
                round: (lobby.round || 1) + 1
            });
        } catch (err) { toast.error("Error starting round"); }
    };
    ...
    const renderBoardGrid = () => {
        ...
        const commitLine = () => {
            ...
            currentLine.forEach((item, i) => { row[startIdx + i] = item; });
            rows.push(row);
            currentLine = [];
        };
        ...
        for (let i = 0; i < workingWords.length; i++) {
            ...
            if (word.length > COLS) {
                commitLine();
                const p1 = word.slice(0, COLS - 1) + "-";
                const p2 = word.slice(COLS - 1);
                p1.split('').forEach(c => currentLine.push({ char: c, isActive: true, isRevealed: c === '-' || lobby.revealedLetters.includes(c) }));
                commitLine();
                workingWords.splice(i + 1, 0, p2);
                continue;
            }
            ...
            if (currentLine.length + spaceNeeded + word.length <= COLS) {
                ...
                word.split('').forEach(c => currentLine.push({ char: c, isActive: true, isRevealed: lobby.revealedLetters.includes(c) }));
            } else {
                commitLine();
                word.split('').forEach(c => currentLine.push({ char: c, isActive: true, isRevealed: lobby.revealedLetters.includes(c) }));
            }
        ...
        <div className="min-h-screen w-full bg-memoryes-background text-memoryes-clay flex flex-col items-center justify-between pb-10 relative overflow-hidden">
            ...
            <header className="w-full p-6 pt-12 flex justify-between items-center bg-white/50 backdrop-blur-md border-b border-slate-100 z-50">
                ...
                <div className="flex flex-col items-center">
                    <span className="text-[9px] font-black uppercase tracking-[3px] text-slate-300">Round {lobby.round || 1}/{lobby.maxRounds || 3}</span>
            ...
            <section className="w-full px-6 mt-4 flex gap-3 overflow-x-auto no-scrollbar py-2">
                {lobby.players.map((name, idx) => (
                    <motion.div key={idx} animate={{ scale: idx === lobby.activePlayerIdx ? 1.05 : 1, backgroundColor: idx === lobby.activePlayerIdx ? '#FFF' : 'rgba(255,255,255,0.4)', borderColor: idx === lobby.activePlayerIdx ? '#9B86BD' : 'transparent' }} className="min-w-[140px] p-4 rounded-[2.2rem] border-2 shadow-sm flex flex-col items-center">
                        <p className="text-[10px] font-bold truncate w-full text-center mb-1">{name}</p>
                        <p className="text-sm font-black text-memoryes-primary">{lobby.playerScores[idx]}$</p>
                        <div className="mt-1 pt-1 border-t border-slate-100 flex items-center gap-1 opacity-50">
                            <Coins size={10} /> <span className="text-[9px] font-bold uppercase">Bank: {lobby.playerTotalScores[idx]}$</span>
                        </div>
                    </motion.div>
                ))}
            </section>
                ...
                <div className="bg-memoryes-clay p-6 rounded-[3.5rem] shadow-2xl space-y-1.5 border-8 border-white/10">
                    {boardGrid.map((row, rIdx) => (
                        <div key={rIdx} className="flex justify-center gap-1">
                            {row.map((tile, tIdx) => (
                                <div key={tIdx} style={{ width: `7.5%` }} className={`aspect-[3/4] flex items-center justify-center rounded-md transition-all duration-500 shadow-sm ${tile.isActive ? tile.isRevealed ? 'bg-white text-memoryes-clay shadow-lg' : 'bg-memoryes-soft/20 border border-white/10' : 'opacity-0'}`}>
                                    {tile.isRevealed && <span className="font-black text-xs md:text-sm">{tile.char}</span>}
                                </div>
                            ))}
                        </div>
                    ))}
                    <div className="pt-6 text-center"><span className="bg-white/10 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[5px] text-white/40">{lobby.category}</span></div>
                </div>
            </section>
                        ...
                        <motion.div key="my-turn" initial={{ y: 20 }} animate={{ y: 0 }} className="space-y-4">
                            {lobby.turnState === 'idle' && (
                                <div className="grid grid-cols-1 gap-3">
                                    <GameBtn onClick={() => pushStateUpdate({ turnState: 'spinning' })} icon={<Sparkles size={20} />} label="Spin the Wheel" color="bg-memoryes-primary text-white" />
                                    <div className="grid grid-cols-2 gap-3">
                                        <GameBtn onClick={() => pushStateUpdate({ turnState: 'buying_vowel' })} icon={<Quote size={18} />} label="Buy Vowel" color="bg-white border border-slate-100 shadow-sm" disabled={lobby.playerScores[lobby.activePlayerIdx] < 250} />
                                        <GameBtn onClick={() => pushStateUpdate({ turnState: 'solving' })} icon={<HelpCircle size={18} />} label="Solve Mode" color="bg-memoryes-clay text-white shadow-sm" />
                                    </div>
                                </div>
                            )}

                            {lobby.turnState === 'spinning' && <Wheel onSpinComplete={handleSpinComplete} isSpinning={true} setIsSpinning={() => { }} />}
                                ...
                                <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-white p-6 rounded-[2.5rem] shadow-2xl border border-slate-100">
                                    <div className="flex justify-between items-center mb-4 px-2">
                                        <div className="flex items-center gap-2">
                                            {lobby.turnState === 'solving' && <Zap size={14} className="text-amber-500 fill-amber-500 animate-pulse" />}
                                            <span className="text-[10px] font-black uppercase tracking-[3px] text-slate-300">
                                                {lobby.turnState === 'spinned' ? `Pick Consonant (${lobby.currentWheelValue}$)` : lobby.turnState === 'buying_vowel' ? 'Choose Vowel' : 'Streak Mode: Pick Letters'}
                                            </span>
                                        </div>
                                        <button onClick={() => pushStateUpdate({ turnState: 'idle' })} className="text-slate-200"><RefreshCcw size={16} /></button>
                                    </div>
                ...
                {(lobby.turnState === 'game_over' || lobby.turnState === 'round_over') && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] bg-memoryes-clay/90 backdrop-blur-md flex items-center justify-center p-8">
                        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white p-10 rounded-[4rem] shadow-2xl w-full max-w-sm text-center space-y-8">
                            ...
                            <h2 className="text-3xl font-serif italic text-memoryes-clay leading-tight">{lobby.turnState === 'game_over' ? 'Vault Legend' : 'Round Secured'}</h2>
...
function GameBtn({ onClick, icon, label, color, disabled }: any) {
    return <button disabled={disabled} onClick={onClick} className={`w-full py-5 rounded-[2.2rem] font-bold flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-all disabled:opacity-20 ${color}`}>{icon} <span className="uppercase text-[11px] tracking-[2px]">{label}</span></button>;
}
